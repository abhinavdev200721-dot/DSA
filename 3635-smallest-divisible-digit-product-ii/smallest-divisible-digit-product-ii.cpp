    ...
    string smallestNumber(string num, long long t) {
        ...
        for (int i = num.length() - 1; i >= 0; --i) {
            ...
            for (int biggerDigit = d + 1; biggerDigit < 10; ++biggerDigit) {
                const unordered_map<int, int> factorsAfterReplacement = getFactorCount(
                    substract(substract(primeCount, primeCountPrefix), kFactorCounts.at(biggerDigit))
                );
        ...
        const unordered_map<int, int> factorsAfterExtension = getFactorCount(primeCount);
        return string(num.length() + 1 - sumValues(factorsAfterExtension), '1') + construct(factorsAfterExtension);
    }

private:
    const inline static unordered_map<int, unordered_map<int, int>> kFactorCounts = {
